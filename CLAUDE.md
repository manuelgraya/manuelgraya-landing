...
- `js/shader.js` — fondo animado del index: shader WebGL de Stitch (rejilla pixel pulsante con scanline) en un canvas fijo tras el contenido (`.fondo-shader`). Con prefers-reduced-motion pinta un solo fotograma; sin WebGL se quita el canvas. La pantalla de carga del index (`.cargador`, SVG SMIL + tecleo) es CSS-only y retrasa 1.5s el arranque del efecto TV del hero
